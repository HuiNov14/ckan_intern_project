    ...
    def get_users_statistics(cls, data_dict):
        try:
            ...
            query = meta.Session.query(
                ...
                cls.about, 
                cls.email, 
                cls.sysadmin.label('sys_admin'),
                func.date(cls.created).label('created_date'), 
                func.date(cls.last_active).label('last_active'),
            )
        
        # Các lọc khác vẫn giữ nguyên
            ...
            sys_admin = data_dict.get('sys_admin')
            if sys_admin is not None:
                query = query.filter(cls.sysadmin == sys_admin)

        # Xử lý ngày hoạt động gần nhất
            target_active_date = data_dict.get('target_active_date', datetime.datetime.now())
        ...
        # Chuyển các kết quả query thành từ điển để dễ dàng JSON hóa
            ...
            for row in range(data_dict.get('recent_active_days')):
                ...
                user_count = query.filter(func.date(cls.last_active) == date).count()
                ...
                result.append({
                    ...
                    'user_info_detail': [dict(row) for row in user_info_detail] if user_info_detail else None
                })
                count += user_count
